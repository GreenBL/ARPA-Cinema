...
constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    // seats Drawable
    private var bookDrawable = dev.jahidhasanco.seatbookview.R.drawable.seats_book
    private var bookedDrawable = dev.jahidhasanco.seatbookview.R.drawable.seats_booked
    private var reservedDrawable = dev.jahidhasanco.seatbookview.R.drawable.seats_reserved
    private var selectedDrawable = dev.jahidhasanco.seatbookview.R.drawable.seats_selected
    ...
    init {
        ...
        inflate(context, dev.jahidhasanco.seatbookview.R.layout.layout_view_group, this)
        viewGroupLayout = findViewById(dev.jahidhasanco.seatbookview.R.id.layout_viewGroup)

        getViewSize()

        if (attrs != null) {
            context.withStyledAttributes(attrs, dev.jahidhasanco.seatbookview.R.styleable.SeatBookView, 0, 0) {

                textSize =
                    getDimensionPixelSize(
                        dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_seat_text_size,
                        15
                    ).toFloat() / scaledDensity

                seatSize = (getDimensionPixelSize(
                    dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_seat_size,
                    250
                ) / density).toInt()

                seatGaping = (getDimensionPixelSize(
                    dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_seat_gaping,
                    10
                ) / density).toInt()

                reservedDrawable = getResourceId(
                    dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_reserved_seat_background,
                    reservedDrawable
                )

                bookDrawable =
                    getResourceId(
                        dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_available_seat_background,
                        bookDrawable
                    )

                bookedDrawable =
                    getResourceId(
                        dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_booked_seat_background,
                        bookedDrawable
                    )

                selectedDrawable =
                    getResourceId(
                        dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_selected_seats_background,
                        selectedDrawable
                    )

                reservedTextColor =
                    getColor(
                        dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_reserved_seats_text_color,
                        reservedTextColor
                    )

                bookTextColor =
                    getColor(
                        dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_available_seats_text_color,
                        bookTextColor
                    )

                bookedTextColor =
                    getColor(
                        dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_booked_seats_text_color,
                        bookedTextColor
                    )

                selectSeatLimit = getInt(dev.jahidhasanco.seatbookview.R.styleable.SeatBookView_seat_select_limit, Int.MAX_VALUE)
            }
        }
    ...
    fun isCustomTitle(r: Boolean): CustomSeatView {
        isCustomTitle = r
        return this
    }

    fun setCustomTitle(titles: List<String>): CustomSeatView {
        title = titles
        return this
    }

    fun setSelectSeatLimit(limit: Int): CustomSeatView {
        selectSeatLimit = limit
        return this
    }


    fun setSeatGaping(size: Int): CustomSeatView {
        seatGaping = (size / density).toInt()
        return this
    }

    fun setSeatLayoutPadding(size: Int): CustomSeatView {
        layout_padding = (size / density).toInt()
        return this
    }

    fun setSeatSize(size: Int): CustomSeatView {
    ...
    fun setSeatsLayoutString(seats: String): CustomSeatView {
        this.seats = seats
        return this
    }
    ...
    fun setAvailableSeatsBackground(drawable: Int): CustomSeatView {
    ...
    fun setBookedSeatsBackground(drawable: Int): CustomSeatView {
        bookedDrawable = drawable
        return this
    }

    fun setReservedSeatsBackground(drawable: Int): CustomSeatView {
        reservedDrawable = drawable
        return this
    }

    fun setSelectedSeatsBackground(drawable: Int): CustomSeatView {
    ...
    fun setReservedSeatsTextColor(color: Int): CustomSeatView {
        reservedTextColor = color
        return this
    }

    fun setAvailableSeatsTextColor(color: Int): CustomSeatView {
        bookTextColor = color
        return this
    }

    fun setBookedSeatsTextColor(color: Int): CustomSeatView {
    ...
    fun clearSelection() {
        // Iterate through the seatViewList and reset the background of selected seats
        for (view in seatViewList) {
            // Check if the view is selected (using the selectedIdList)
            if (view.id in selectedIdList) {
                // Mark as available to clear the selection
                markAsAvailable(view)
            }
        }
